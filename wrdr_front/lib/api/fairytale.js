...
export const requestFirstSentence2 = async (charName, bgPlace) => {
  try {
    ...
    const man1 = charName[0]; // man1 = 등장인물1
    const man2 = charName[1]; // man2 = 등장인물2
    let man3 = '';
    if (charName[2]) {
      man3 = charName[2]; //man3 = 등장인물3
    } else {
      ('');
    }
    const bg = bgPlace; // bg = 배경
    let bgCheck;

    if (bg === '숲속') {
      bgCheck = '숲';
    } //bgCheck = 배경이 있나없나 확인하는 요소
    else {
      bgCheck = bg;
    }

    function checkNamePost(name, post) {
      //받침에 따라 은는이가을를 구분
    ...
    // ex = 인물2명 첫문단 api 에서 나온 결과
    let sentence = response.data.result.text;
    //1. 2번째 인물과 배경이 둘다 반영 안된경우
    if (bgCheck !== '바닷속' && !sentence.includes(man2) && !sentence.includes(bgCheck)) {
      sentence = ' ' + sentence + checkNamePost(man1, '은는') + ' 우연히 ' + bg + '에서 ' + checkNamePost(man2, '을를') + ' 마주치게 되었어요.';
    }
    //2. 2번째 인물만 반영 안된 경우
    else if (!sentence.includes(man2)) {
      sentence = sentence + ' 그곳에서 ' + checkNamePost(man1, '은는') + ' ' + checkNamePost(man2, '을를') + ' 마주쳤어요.';
    }

    //3. 배경만 반영 안된 경우
    else if (bgCheck !== '바닷속' && !sentence.includes(bgCheck)) {
      sentence = sentence + '그들이 서로를 마주친 건 ' + bg + '에서였지요.';
    }
    if (man3) {
      sentence = sentence + ' 그곳에는 ' + man3 + '도 함께였답니다.';
    } else {
      ('');
    }
...
const middleTextTest2 =
  '동화의 다음 문장을 생성하시오.\n\n동화: 아주 먼 옛날, 어느 한 아름다운 마을에 나무꾼과 알라딘이 살고 있었어요. 둘은 사이좋은 이웃이었답니다. 어느 날 나무꾼과 알라딘은 처음 보는 골목을 발견했어요. 호기심 많은 둘은 골목으로 들어갔어요. 골목 안으로 들어가니 새로운 세계가 펼쳐졌어요. 작고 아기자기한 상점들이 줄지어 있었고, 각각의 상점마다 신비로운 물건들이 진열되어 있었어요. 그 중에서 알라딘의 눈을 끈 것은 금빛이 나는 작은 램프였어요. 나무꾼은 금도끼에 관심이 있었지요. 각자 자신이 원하는 물건을 사들고 마을로 돌아왔답니다. 그런데 그 다음날부터 나무꾼과 알라딘에겐 이상한 일이 벌어졌어요.\n결과: 바로 집에 있는 물건들이 하나씩 사라지는 것이었지요. 이를 수상하게 여긴 둘은 램프와 금도끼를 들고 마을에 사는 마녀에게 찾아갔어요.\n###\n동화: 머나먼 나라 커다란 성에 공주가 태어났어요. 공주의 이름은 백설공주였지요. 시간이 흘러 공주의 15번째 되던 생일날, 백설공주는 성 밖으로 나갈 수 있다는 허락을 받았어요. 백설공주는 왕국 옆 숲으로 홀로 여행을 떠났답니다. 그 곳에는 아름다운 꽃들과 신비로운 동물들로 가득한 곳이었어요. 백설공주는 나무 밑에서 책도 읽고, 작은 호수에서 물장구도 치며 즐거운 시간을 보냈답니다. 그러다가 백설공주는 따사로운 햇살에 그만 깜빡 잠이 들고 말았어요. 잠에서 깬 백설공주 앞에는 피터팬이 있었어요. 백설공주는 말했어요. "깜짝이야! 넌 누구지?" 피터팬이 대답했어요. "난 숲의 요정 피터팬이야. 이 숲을 안전하게 지키는 일을 하지. 나와 함께 숲속을 둘러보지 않을래?"\n결과: 피터팬의 제안을 받아들인 백설공주는 숲 이곳저곳을 탐험했어요. 그러다가 두 사람은 숲에서 가장 크고 오래된 신비로운 나무 앞에 도착했답니다.\n###\n동화: 먼 옛날 한 아름다운 바닷속 아무도 모르는 도깨비 마을이 있었어요. 그 마을은 마법을 써서 다른 바다 생물들에게는 보이지 않아서 안전했지요. 어느 날 호기심 많은 아기 도깨비는 모두가 잠든 틈을 타 몰래 마을을 빠져나왔어요. 아기 도깨비가 처음 보는 물고기와 해파리, 신기한 해조류들이 넘쳐났어요. 그는 더욱 먼 곳으로 모험을 떠났어요. 그러다가 그는 신비로운 해저 동굴을 발견했어요. \n결과: 동굴 속에서는 반짝이는 보물과 아름다운 조개껍질이 가득했어요. 하지만 동굴 깊은 곳에는 무시무시한 존재가 아기 도깨비를 기다리고 있었어요.\n###\n동화: 깊고 깊은 바닷속 인어공주에게는 다섯 명의 언니들이 있었어요. 언니들은 막내 인어공주에게 바다 위 세상에 대한 이야기를 매일매일 해주었죠. 바다 위 세상에 대한 이야기를 들을 때마다 인어공주의 가슴은 두근두근 뛰었답니다. 그리고 드디어 공주의 열다섯 번째 생일날 열다섯 살이 된 인어공주는 바다 위로 힘차게 헤엄쳤어요. 그때 흥겨운 음악 소리와 함께 배 한 척이 다가왔어요. 인어공주는 배 갑판에 서 있는 왕자님을 본 순간 숨을 쉴 수가 없었어요.\n결과: 왕자님에게 한 눈에 반해버리고 만 인어공주는 그 이후로 왕자님을 볼 수 없게 되자 시름시름 앓았답니다.\n###\n동화:';
